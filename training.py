...
from gather import gather as gather_all
from models.HCURec import HCURec, create_model
from utils.train_util import set_seed
from utils.train_util import save_checkpoint_by_epoch


def run(cfg, rank, device, finished, train_dataset_path, valid_dataset_file, user_emb, seed):
    """
    train and evaluate
    :param args: config
    ...
    if (cfg.gpus < 2) or (cfg.gpus > 1 and rank == 0):
        ...
        gather_all(cfg.result_path, 1, validate=False, save=False)
    ...
    for epoch in range(cfg.epoch):
        ...
        if (cfg.gpus < 2) or (cfg.gpus > 1 and rank == 0):
            ...
            while finished.value < cfg.gpus:
                time.sleep(1)
            gather_all(cfg.result_path, cfg.gpus, validate=False, save=False)
            finished.value = 0
...
def train(cfg, epoch, rank, model, loader, optimizer, steps_one_epoch, device):
    ...
    for i, data in enum_dataloader:
        ...
        # 1. Forward
        pred = model(data[:, 2:], start_point, device).squeeze()
...
def validate(cfg, epoch, model, device, rank, valid_data_loader, fast_dev=False, top_k=20):
    ...
    with torch.no_grad():
        ...
        for i, data in data_iter:
            ...
            # 1. Forward
            pred = model(data[:, 2:], start_point, device, test_mode=True)
            if pred.dim() > 1:
                pred = pred.squeeze()
            try:
...
def init_processes(cfg, local_rank, vocab, dataset, valid_dataset, user_emb, finished, fn, backend='nccl', seed=7):
    """ Initialize the distributed environment. """
    addr = "localhost"
    port = cfg.port
    os.environ['MASTER_ADDR'] = addr
    os.environ['MASTER_PORT'] = str(port)
    dist.init_process_group(backend='gloo', rank=0 + local_rank,
                            world_size=cfg.gpus)

    device = torch.device("cuda:{}".format(local_rank))

    fn(cfg, local_rank, device, finished, train_dataset_path=dataset, valid_dataset_file=valid_dataset,
       user_emb=user_emb, seed=seed)
...
def main(cfg):
    ...
    for rank in range(cfg.gpus):
        p = mp.Process(target=init_processes, args=(
            cfg, rank, None, "{}/raw/train-{}-new.npy".format(cfg.root, rank), valid_dataset_list[rank], user_emb,
            finished, run, "nccl", seed))
        p.start()
        processes.append(p)
