...
def main(cfg):
    ...
    model_cfg.mask_prob = -1
    cfg.mc = model_cfg
    user_emb = np.load('{}/user_emb.npy'.format(cfg.root))

    model = create_model(model_cfg)

    saved_model_path = os.path.join(cfg.root, 'checkpoint', 'model.ep{0}'.format(cfg.epoch - 1))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--epoch', type=int, default=10, help='the number of epochs load checkpoint')
